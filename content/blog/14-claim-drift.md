...
```ts
// Illustrative shape. Live integers stay in the claim-drift gate.
// This essay does not copy them, so a later commit cannot make the essay the stale copy.
...
A page that wants to state a component count imports `METRICS.uiComponents`. It never types the integer by hand. Change the underlying number in one place and the copy follows automatically, on the marketing site, in the docs, and in the product roadmap, with no copy edit at all.
...
That hard failure is the whole point. The numbers on the product site are not a snapshot someone updated when they remembered. They are a measurement of the code on the commit that publishes them. This essay does not repeat those counts.
...
*RevealUI is the open runtime for businesses that run their own AI. Every claim on the product site is checked against the source. Read it in the [docs](https://docs.revealui.com).*
