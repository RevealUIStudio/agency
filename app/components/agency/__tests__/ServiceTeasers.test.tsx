...
describe('ServiceTeasers', () => {
  it('renders only the three locked studio offers', () => {
    ...
    expect(screen.getByText(ARCHITECTURE_ARTIFACT_BUNDLE.name)).toBeInTheDocument();
    ...
    expect(container.textContent ?? '').toContain(ARCHITECTURE_ARTIFACT_BUNDLE.payment);
  ...
  it('anchors each offer and points CTAs at the intro calendar', () => {
    ...
    expect(document.getElementById('architecture-artifact-bundle')).not.toBeNull();
    expect(document.getElementById('written-plan')).toBeNull();
